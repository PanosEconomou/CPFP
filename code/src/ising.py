...
class field:
    ...
    def quadratic(self,x): return x**2
    def order3(self,x): return x**3
    def order4(self,x): return x**4
    def equal(self,x):
        out = []
        for i in x:
        ...
        return np.array(out)
    def root(self,x): return (abs(x))**0.5
    def fractional(self,x): return (1/(1-0.1*x)-1)*0.25
    def partitioned(self,x):
        out = []
        for i in x:
            if i > 0:
                out.append(1.)
