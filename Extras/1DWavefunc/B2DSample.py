...
def getPsi0(axes,k = None,C:float = 1, x0=None, s0 = 1e-2**0.5):
    N = int(len(axes[0]))
    if k == None:
        k = np.zeros(dim)
        k[1] = 100
...
def stepImag(R,I,V,dx,dt,axes):
    ...
    for i in range(1,len(axes[0])-1):
        for j in range(1,len(axes[1])-1):
            ...
            if 0.15 < axes[1][j] < 0.4:
                B = 1e5
            else:
                B = 0

            DIx = (I[i+1][j]-I[i-1][j])/(2*dx)
            DIy = (I[i][j+1]-I[i][j-1])/(2*dx)

            T = (q/m)*dt*B*(axes[0][i]*DIy-axes[1][j]*DIx)

            Inew[i][j] = I[i][j] + h*dt/(2*m*dx**2)*S +\
                        B*dt*R[i][j]/2*s -\
                        (q/h)*V[i][j]*dt*R[i][j]
                        #q**2*dt/(2*m*h)*B**2*(axes[0][i]**2+axes[1][j]**2)*R[i][j] +\
                        #T -\

    #Do the boundary
    for i in [0,-1]:
        for j in [0,-1]:
            Inew[i][j] = Inew[3*i+1][3*j+1]
...
def stepReal(R, I, V, dx, dt, axes):
    ...
    for i in range(1, len(axes[0])-1):
        for j in range(1, len(axes[1])-1):
            ...
            DRx = (R[i+1][j]-R[i-1][j])/(2*dx)
            DRy = (R[i][j+1]-R[i][j-1])/(2*dx)

            T = (q/m)*B*(axes[0][i]*DRy-axes[1][j]*DRx)*dt

            # Rnew[i][j] = R[i][j] - h*dt/(2*m*dx**2)*S + (1/h)*V[i][j]*dt*I[i][j]
            Rnew[i][j] = R[i][j] - h*dt/(2*m*dx**2)*S -\
                        B*dt*I[i][j]/2*s +\
                        (q/h)*V[i][j]*dt*I[i][j]
            #q**2*dt/(2*m*h)*B**2*abs(axes[0][i]**2+axes[1][j]**2)*I[i][j] -\
            #T +\

    #Do the boundary
    for i in [0, -1]:
        for j in [0, -1]:
            Rnew[i][j] = Rnew[3*i+1][3*j+1]
